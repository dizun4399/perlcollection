...
$ver=4.30;#2003-4-25 11:53 任意位置文字
...
my @stict_keys=("XStart","YStart","XEnd","YEnd","XStep","YStep");
{
	...
	if (!$ok) {
		...
		$param{XUnit}=$XUnit if (!defined($param{XUnit}) && (!defined($param{Type}) || $param{Type} eq 'Rect'));
		seek(F,$fpos,0);
	}
}
#####################################################################################################################
...
$param{Type}=~/\S+/;
$type=$&;
$type=$type[0] if (!defined($type) || $type eq '' || $type eq 'Rect' || $type eq 'Rectangle');
...
sub draw{
	...
	$alignPos=$tpos{uc($draw{AlignPos})} ? uc($draw{AlignPos}) : 'LT';
		...
		foreach (@x) {
			...
			foreach $i (keys %{$xy{$_}}) {
				...
				if ($drawType =~ /$type[2]/) { #Line
					...
					if ($shape eq 'circle') {
						$g->d("circle","cx",fx($x),'cy',fy($y),'r',$pointSize,"style",style($d{circle}),
							"onclick","alert('x:$tmpx\\ny:$tmpy".($maxDot>1 ? "\\ndot:$dotNum" : '')."$popNote')",
							"onmousemove","window.status='x:$tmpx\\ty:$tmpy".($maxDot>1 ? "\\tdot:$dotNum" : '')."$statusNote'");
					}else{
				...
				if ($xy{$_}{$i}{note} ne '') { #Text $drawType =~ /$type[5]/
					...
					my $tmpalign=$tpos{uc($lnote[3])} ? uc($lnote[3]) : $alignPos;
					$d{font}{fill}=$lcolor;
					$d{font}{'font-size'}=$lnote[1] if ($lnote[1] ne '');
					$d{font}{'font-family'}=$lnote[2] if ($lnote[2] ne '');
					$d{font}{'font-size'}*=$draw{TextScale} if ($draw{TextScale});
					$g->setFontSize($d{font}{'font-size'});
					$g->setFontFamily($d{font}{'font-family'});
					$g->d('txt'.$tmpalign,$lnote[0],'xval',fx($x),'yval',fy($y),"style",style($d{font}));
					#print "$g->d('txt'.$tmpalign,$lnote[0],'xval',fx($x),'yval',fy($y),\"style\",style($d{font}));\n";
					#print "$g->d('txt'.$tmpalign,$lnote[0],'xval',fx($x),'yval',fy($y),\"style\",style($d{font}));\n";
